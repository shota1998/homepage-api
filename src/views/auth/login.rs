...
///
/// # Arguments
/// credentials(Json::<Login>): Includes name and password.
///
/// # Returns
/// (HttpResponse) Whether login was succeeded or not.
pub async fn login(credentials: web::Json<Login>) -> HttpResponse {
    ...
    let users = users::table.filter(users::columns::username.eq(username.as_str()))
